...
class Mass:
	...
	def writeto(self, filehandle, typedict=None):
		if typedict:
			try:
				print("Mass, type", self.nr, self.mass, self.type, typedict[self.type])
...
class Atom:
	...
	def writeto(self, filehandle, namedict=None, type_to_name=None):
		if type_to_name:
			atomname = namedict[self.name]

			atomtype = self.type

			for an, at in type_to_name[self.type]:
				if(an == atomname):
					atomtype = at
		...
		print("Atom writeto ", atomtype, atomname)
		filehandle.write( 'ATOM %-7s%-7s %s %s \n' %(atomname, atomtype, self.charge, self.memo) )
...
def possible_type_conversions(l):
	conversions = set()
	for an, at in l: 
		conversions.add(at)
...
class BondParams:
	...
	def writeto(self, filehandle, type_to_name=None):
		type_pairs = []
		if type_to_name:
			try:
				# type1 = typedict[self.type1]
				# type2 = typedict[self.type2]
				c1 = possible_type_conversions(type_to_name[self.type1])
				c2 = possible_type_conversions(type_to_name[self.type2])
				for t1 in c1:
					for t2 in c2:
						if((t1, t2) not in type_pairs and (t2, t1) not in type_pairs):
...
def createdicts(atomlist, mapping):
	...
	namedict, typedict, type_to_name = {}, {}, {}
	for i, a in enumerate(atomlist):
		print("createdicts", i, a.type, a.name, mapping[a].type, mapping[a].name)
		typedict[a.type] = mapping[a].type
		namedict[a.name] = mapping[a].name

		if(a.type not in type_to_name):
			type_to_name[a.type] = [(a.name, mapping[a].type)]
		else:
			type_to_name[a.type].append((a.name, mapping[a].type))

	return namedict, typedict, type_to_name	
...
def writertf(filehandle, namedict, typedict):
	...
	for ac in atomlist:
		...
		ac.writeto(filehandle, namedict, type_to_name)
	for bc in bondlist:
...
def writepar(filehandle, namedict, typedict):
	...
	for b in bonparams:
		...
		b.writeto(filehandle, type_to_name)
	filehandle.write( '\nANGLES\n' ) 	
	for a in anglparams:
...
Optionally, place a stream file obtained from paramchem in the viparr3 directory to change atom names and types.')
	...
	if mapping:
		namedict, typedict, type_to_name = createdicts(atomlist, mapping)

		for n in namedict:
			print("namedict", n, namedict[n])

		for k in typedict:
			print("typedict", k, typedict[k])

		for g in type_to_name:
			print("type_to_name", g, type_to_name[g])
	else:
