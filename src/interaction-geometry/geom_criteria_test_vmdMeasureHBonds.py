...
def debugRepeats(frame_index, dist_cutoff, angle_cutoff):
	donors, acceptors, hydrogens = calcDonorAcceptors(frame_index, None, dist_cutoff, angle_cutoff)
	donors, acceptors, hydrogens = removeDuplicateIndices(donors, acceptors, hydrogens)
	key_pairs = []
	for i, d in enumerate(donors):
		key_pairs.append((donors[i], acceptors[i], hydrogens[i]))
	key_dict = {}
	for a in key_pairs:
...
def removeDuplicateIndices(donors, acceptors, hydrogens):
	pairs = []
	for i, d in enumerate(donors):
		pairs.append((donors[i], acceptors[i], hydrogens[i]))
	pairs = sorted(list(set(pairs)))
	newDonors, newAcceptors, newHydrogens = [], [], []
	for p in pairs:
		newDonors.append(p[0])
		newAcceptors.append(p[1])
		newHydrogens.append(p[2])
	return newDonors, newAcceptors, newHydrogens
...
def genHBondsAtomSelection(frame_index, dist_cutoff, angle_cutoff, chainId, ligand, solvent_id):
	print("genHBondsAtomSelection() " + str(ligand) + " dist_cutoff " + str(dist_cutoff) + " angle_cutoff " + str(angle_cutoff))
	if(ligand == None):
		if(solvent_id == "IP3"):
			if(chainId == None):
				print("try this01")
...
def calcDonorAcceptors(frame_index, dist_cutoff, angle_cutoff, chainId=None, ligand=None):
	...
	list3 = lists[2].split("{")[1].split(" ")
	hydrogens = [int(a) for a in list3]
	return donors, acceptors, hydrogens
...
def calcHBondFrame(f, frame_index, dist_cutoff, angle_cutoff, lookup_table, chainId=None, ligand=None, crystal=False):
	...
	if(ligand == None):
		donors, acceptors, hydrogens = calcDonorAcceptors(frame_index, dist_cutoff, angle_cutoff, chainId) #indices 
	else:
		donors, acceptors, hydrogens = calcDonorAcceptors(frame_index, dist_cutoff, angle_cutoff, chainId, ligand) #indices 
	donors, acceptors, hydrogens = removeDuplicateIndices(donors, acceptors, hydrogens) # Remove duplicates 
	donor_label = indexToLabel(frame_index, donors, crystal)
	acceptor_label = indexToLabel(frame_index, acceptors, crystal)
	for i in range(len(donors)):
		d_atom, a_atom, h_atom = donors[i], acceptors[i], hydrogens[i]
		d = evaltcl("measure bond {" + str(d_atom) + " " + str(a_atom) + "}")
		ang = evaltcl("measure angle {" + str(d_atom) + " " + str(h_atom) + " " + str(a_atom) + "}")
		print("whatsup? ", donor_label[i], acceptor_label[i], d_atom, a_atom, d, ang)
