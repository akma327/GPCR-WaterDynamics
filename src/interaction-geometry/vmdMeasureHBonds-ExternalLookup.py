# vmdMeasureHBonds-ExternalLookup.py 
...
# Usage: python vmdMeasureHBonds-ExternalLookup.py  <TRAJECTORY_PATH> <TOPOLOGY_PATH> <LOOKUP_TABLE_PATH> <OUTPUT_FILE> <Optional -chain flag> <Chain> <Optional -solv flag> <solvent_id> 
...
# <LOOKUP_TABLE_PATH> Absolute path to the external sqlite3 lookup table 
...
# LOOKUP_TABLE_PATH="/scratch/PI/rondror/akma327/DynamicNetworks/data/Top-Atom-Lookup-Tables/B2AR-active-Gs-BI-science2015-cond22.db"
...
# python vmdMeasureHBonds-ExternalLookup.py $TRAJ_PATH $TOP_PATH $LOOKUP_TABLE_PATH $OUTPUT_FILE -chain R 
...
# LOOKUP_TABLE_PATH="/scratch/PI/rondror/akma327/DynamicNetworks/data/Top-Atom-Lookup-Tables/B2AR-active-Gs-BI-science2015-cond22.db"
# OUTPUT_FILE="/scratch/PI/rondror/akma327/DynamicNetworks/data/DynamicNetworksOutput/InteractionOutput/B2AR-active-Gs-BI-science2015/condition-22/rep_1/Prod_0/HB_3.5A_70D/hydrogen_bond_water_result.txt"
# sys.argv = ['a', TRAJ_PATH, TOP_PATH, LOOKUP_TABLE_PATH, OUTPUT_FILE, '-chain', 'R']
...
def getFromLookupTable(lookup_table, key):
	for row in lookup_table.execute('SELECT * FROM atom_lookup WHERE atom_index= ?', (key,)):
		return row[1]

	return None 

def indexToLabel(lookup_table, hbond_index_list, crystal=False):
	label_list = []
	for atom_index in hbond_index_list:
		key = int(atom_index)
		atom_label = getFromLookupTable(lookup_table, key)
		if(atom_label == None):
			print("key: " + str(key) + " not found.")
			# exit(1)
		else:
			print(key, atom_label)
...
def calcHBondFrame(f, frame_index, dist_cutoff, angle_cutoff, lookup_table, chainId=None, crystal=False):
	crystalHBond = []
	print("calcHBondFrame() for frame_index: " + str(frame_index))
	...
	donor_label = indexToLabel(lookup_table, donors, crystal)
	acceptor_label = indexToLabel(lookup_table, acceptors, crystal)
...
def calcVMDHydrogenBondWaterResults(f, stride_val, TOP_PATH, TRAJ_PATH, LOOKUP_TABLE_PATH, solventId, chainId=None, crystal=False, dist_cutoff=3.5, angle_cutoff=70):
	global solvent_id
	global stride
	if(solventId != None):
	...
	stride = stride_val 
	tic = time.clock()
	conn = sqlite3.connect(LOOKUP_TABLE_PATH)
	lookup_table = conn.cursor() 
	load_traj(TOP_PATH, TRAJ_PATH)
	numFrames = int(evaltcl('molinfo top get numframes'))
	if(crystal == True):
		crystalHBond = calcHBondFrame(f, 0, dist_cutoff, angle_cutoff, lookup_table, chainId, crystal)
		return crystalHBond
	else:
...
if __name__ == "__main__":
	...
	calcVMDHydrogenBondWaterResults(f, 1, TOP_PATH, TRAJ_PATH, LOOKUP_TABLE_PATH, solvent_id, chainId)
