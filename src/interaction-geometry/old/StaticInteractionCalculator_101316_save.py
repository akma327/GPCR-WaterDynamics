...
def calcHydrogenBondWaterResults(file_writer, TOP_PATH, solventId, chainId=None, ligand=None):
	print("\n\nHydrogen Bond-Water Mediated:" + TOP_PATH + "\n")
	tic = time.clock()
	hbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, None, True)
...
def calcLigandHydrogenBondWaterResults(file_writer, TOP_PATH, solventId, chainId=None, ligand=None):
	print("\n\nLigand Hydrogen Bond-Water Mediated:" + TOP_PATH + "\n")
	tic = time.clock()
	lhbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, ligand, True)
...
def calcBackBoneHBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	print("\n\nBackbone Backbone Hydrogen Bonds:" + TOP_PATH + "\n")
	tic = time.clock()
	hbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, None, crystal =True)
	print(hbwFramePairs)
	for resAtom1, resAtom2 in hbwFramePairs:
		resAtom1, resAtom2 = str(resAtom1), str(resAtom2)
		if(solvent_id not in resAtom1 and solvent_id not in resAtom2):
...
def calcLigandBackBoneHBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	print("\n\nLigand Backbone Hydrogen Bonds:" + TOP_PATH + "\n")
	tic = time.clock()
	lhbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, ligand, crystal =True)
	print(lhbwFramePairs)
	for resAtom1, resAtom2 in lhbwFramePairs:
		resAtom1, resAtom2 = str(resAtom1), str(resAtom2)
		if("LIG" in resAtom1 and ("LIG" not in resAtom2 and solvent_id not in resAtom2)): 
			print("hi", resAtom1, resAtom2)
			atom2 = resAtom2.split("-")[1].strip()
			if(atom2 in backbone_atoms):
				file_writer.write(resAtom1 + " -- " + resAtom2 + "@-hlb\n")
		if(("LIG" not in resAtom1 and solvent_id not in resAtom1) and "LIG" in resAtom2):
...
def calcSideChainBackBoneHBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
...
def calcLigandSideChainHBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
...
def calcSideChainHBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
...
def calcResidueWaterBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	print("\n\nResidue Water Hydrogen Bonds:" + TOP_PATH + "\n")
	tic = time.clock()
	hbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, None, crystal=True)
	for resAtom1, resAtom2 in hbwFramePairs:
		resAtom1, resAtom2 = str(resAtom1), str(resAtom2)
		if((solvent_id in resAtom1 and solvent_id not in resAtom2) or (solvent_id not in resAtom1 and solvent_id in resAtom2)):
...
def calcLigandToWaterBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	print("\n\nLigand Water Hydrogen Bonds:" + TOP_PATH + "\n")
	tic = time.clock()
	lhbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, ligand, crystal=True)
	for resAtom1, resAtom2 in lhbwFramePairs:
		resAtom1, resAtom2 = str(resAtom1), str(resAtom2)
		if((solvent_id in resAtom1 and "LIG" in resAtom2) or ("LIG" in resAtom1 and solvent_id in resAtom2)):
...
def calcWaterBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):

	print("am I here yet", solvent_id, solventId)
	...
	for resAtom1, resAtom2 in hbwFramePairs:
		resAtom1, resAtom2 = str(resAtom1), str(resAtom2)
		if(solvent_id in resAtom1 and solvent_id in resAtom2): continue 
		if(not(solvent_id not in resAtom1 and solvent_id not in resAtom2)):
			if(solvent_id in resAtom1):
	...
	print("this makes no sense", frameWBondDict)
	writeToWBondFile(file_writer, frameWBondDict)


def calcLigandWaterBondResults(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	...
	for resAtom1, resAtom2 in lhbwFramePairs:
		resAtom1, resAtom2 = str(resAtom1), str(resAtom2)
		print("dd", resAtom1, resAtom2)
		if(solvent_id in resAtom1 and solvent_id in resAtom2): continue 
		if(not(solvent_id not in resAtom1 and solvent_id not in resAtom2)): #at least one of atoms is water
			if(solvent_id in resAtom1):
...
# water bridge. 
def calcWaterBond2Results(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	frameWBond2Dicts = ({}, {})
	hbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, None, crystal=True)
	for resAtom1, resAtom2 in hbwFramePairs:
		if(solvent_id in resAtom1 and solvent_id in resAtom2):
			if(resAtom1 not in frameWBond2Dicts[0]):
				frameWBond2Dicts[0][resAtom1] = [resAtom2]
			else:
				frameWBond2Dicts[0][resAtom1].append(resAtom2)
		elif((solvent_id in resAtom1 and solvent_id not in resAtom2) or (solvent_id not in resAtom1 and solvent_id in resAtom2)):
			if(solvent_id in resAtom1): solvent_atom, resid_atom = resAtom1, resAtom2
...
def calcLigandWaterBond2Results(file_writer, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
	frameLWBond2Dicts = ({}, {})
	lhbwFramePairs = calcVMDHydrogenBondWaterResults(file_writer, 1, TOP_PATH, None, solventId, chainId, ligand, crystal=True)
	for resAtom1, resAtom2 in lhbwFramePairs:
		if(solvent_id in resAtom1 and solvent_id in resAtom2):
...
def calcStaticInteractions(file_writer, interaction_selection, TOP_PATH, solvent_id="HOH", chainId=None, ligand=None):
...
if __name__ == "__main__":
	...
	for interaction_selection in interaction_selections:
		print("Processing Interaction " + interaction_selection)
