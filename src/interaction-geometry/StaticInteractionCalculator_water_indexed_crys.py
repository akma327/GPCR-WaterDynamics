...
# Date: 04/04/17
# StaticInteractionCalculator_water_indexed_crys.py
...
USAGE_STR = """ 
# Purpose
# Only use this version of StaticInteractionCalculator for competing network analysis where you would need to know the
# Explicit water index in addition to residue index. 

# Usage:
# python StaticInteractionCalculator_water_indexed_crys.py <TOP_PATH> <OUTPUT_FILE_PATH> <-pdb flag> <PDB CODE> <-append flag> <optional -solv flag> <solvent> <optional -chain> <chainID> <optional -ligand> <Ligand> <-interlist> <INTERACTION LIST>
...
# Example DOR_inactive 

TOP_PATH="/scratch/PI/rondror/akma327/DynamicNetworks/data/crystal-analysis/competing-network-analysis/pymol-visualization/040417/DOR_inactive/01-pdb/dor_inactive_prod9_frame127.pdb"
OUTPUT_FILE_PATH="/scratch/PI/rondror/akma327/DynamicNetworks/data/crystal-analysis/competing-network-analysis/pymol-visualization/040417/DOR_inactive/02-raw-hbond/dor_inactive_prod9_frame127.txt"
cd /scratch/PI/rondror/akma327/DynamicNetworks/src/interaction-geometry
python StaticInteractionCalculator_water_indexed_crys.py $TOP_PATH $OUTPUT_FILE_PATH -solv TIP3 -chain A -ligand EJ4 -interlist -wb -wb2 -lwb -lwb2
...
def getWBondCombos(water, resid_list):
	...
	for pair in itertools.combinations(resid_list, 2):
		resid1, resid2 = pair 
		if(resid1 != resid2):
			wBondCombos.add((resid1, resid2, water))
	return wBondCombos
...
def writeToWBondFile(fwb, frameWBondDict):
	...
	print(frameWBonds)
	for resid1, resid2, water in frameWBonds:
		fwb.write(resid1 + " -- " + resid2 + " -- " + water + "@-wb\n")


# Generate combinations of first order ligand water bridges
def getLWBondCombos(water, ligand, resid_ligand_list):
	lwBondCombos = set()
	for resid in resid_ligand_list:
		if("LIG" not in resid): # valid residue
			lwBondCombos.add((ligand, resid, water))
	return lwBondCombos
...
def writeToLWBondFile(fwb, frameLWBondDict):
	...
	for water in frameLWBondDict:
		...
		frameLWBonds = frameLWBonds | getLWBondCombos(water, ligand, resid_ligand_list)
	frameLWBonds = sorted(list(frameLWBonds))
	for ligand, resid, water in frameLWBonds:
		fwb.write(ligand + " -- " + resid + " -- " + water + "@-lwb\n")
...
def residueCombinations(water1, water2, resid_list1, resid_list2):	
	return set([(res1, res2, water1, water2) for res1 in resid_list1 for res2 in resid_list2]) #doesn't make distinct by water 
	...
	for rl1 in resid_ligand_list1:
		for rl2 in resid_ligand_list2:
			if("LIG" in rl1 and "LIG" not in rl2):
				rlCombos.add((rl1, rl2, water1, water2))
			elif("LIG" not in rl1 and "LIG" in rl2):
				rlCombos.add((rl2, rl1, water2, water1))
	return rlCombos
...
def writeToExtendedWBondFile(fwb, frameWBond2Dicts):
	...
	for resid1, resid2, water1, water2 in extendWaterBridges:
		fwb.write(resid1 + " -- " + resid2 + " -- " + water1 + " -- " + water2 + "@-wb2\n")
...
def writeToExtendedLWBondFile(fwb, frameLWBond2Dicts):
	...
	for ligand, resid, water1, water2 in extendLigandWaterBridges:
		fwb.write(ligand + " -- " + resid + " -- " + water1 + " -- " + water2 + "@-lwb2\n")
