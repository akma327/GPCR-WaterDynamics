...
#####################################################################
### Program to align the frames of a trajectory in DESRES trajectories
#####################################################################

# Usage
# python wrapAlign.py <TOP_FILE> <TRAJ_FILE> <OUTPUT_FILE> <-crys flag>
...
if __name__ == "__main__":
	tic = time.clock()
	if(len(sys.argv) <4):
		print "Usage: python wrapAlign.py <topology file> <trajectory file> <out wrapped trajectory file>"
		sys.exit()
	...
	molecule.read(traj_molid, 'dcd', traj_file, beg = 0, end = -1, waitfor = -1)
