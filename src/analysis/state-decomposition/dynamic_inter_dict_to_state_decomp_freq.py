...
# dynamic_inter_dict_to_state_decomp_freq.py
...
# of every frame (active, I1, I2, I3, inactive).

# Output Format:
# GPCR-DB1 GPCR-DB2 <interaction type> <Inactive freq> <Intermediate 1 freq> <Intermediate 2 freq> <Intermediate 3 freq> <Active freq>

# Usage:
# python dynamic_inter_dict_to_state_decomp_freq.py <PDB_CODE> <EXP_REP_DICT_PATH> <STATE_DECOMP_FILE> <OUTPUT_FILE_PATH> <-chainId> <MDTRAJ chain number> <-interlist> <INTERACTION_LIST>
...
python dynamic_inter_dict_to_state_decomp_freq.py $PDB_CODE $EXP_REP_DICT_PATH $STATE_DECOMP_FILE $OUTPUT_FILE_PATH -chainId 0 -interlist -sb -pc -ps -ts -vdw -hbbb -hbsb -hbss -wb -wb2
...
def parseInteractionDictionary(fw, state_decompositions, interaction_type, EXP_REP_INTERACTION_DICT_PATH, gpcrDbDict, CHAIN_ID=None):
...
def interDictToFreqDriver(EXP_REP_DICT_PATH, STATE_DECOMP_FILE, OUTPUT_FILE_PATH, INTERACTION_LIST, gpcrDbDict, CHAIN_ID=None):
	...
	for interaction_type in INTERACTION_LIST:
		...
		parseInteractionDictionary(fw, state_decompositions, interaction_type, EXP_REP_INTERACTION_DICT_PATH, gpcrDbDict, CHAIN_ID)
	toc = time.clock()
	return toc - tic
