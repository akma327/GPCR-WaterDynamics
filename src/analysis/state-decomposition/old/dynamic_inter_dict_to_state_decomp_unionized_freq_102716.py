...
# Usage:
# python dynamic_inter_dict_to_state_decomp_unionized_freq.py <PDB_CODE> <EXP_REP_DICT_PATH> <STATE_DECOMP_FILE> <OUTPUT_FILE_PATH> <-chainId> <MDTRAJ chain number> <-network> <Polar network list> <-interlist> <INTERACTION_LIST>
...
def genRawInteractionBinaryMatrix(numRows, numKeys, interaction_type, EXP_REP_INTERACTION_DICT_PATH, gpcrDbDict, CHAIN_ID = None, POLAR_NETWORK_PATH = None):
	...
	def polar_network_gpcrdb(POLAR_NETWORK_PATH):
		if(POLAR_NETWORK_PATH == None): return None
		pn_gpcrdb_pairs = []
		pn_pdb = None
...
def parseStateFrequencies(fw, rowLabels, rawBinaryMatrix, state_decompositions, POLAR_NETWORK_PATH):
...
def parseInteractionDictionary(state_decompositions, interaction_type, EXP_REP_INTERACTION_DICT_PATH, gpcrDbDict, CHAIN_ID=None, POLAR_NETWORK_PATH = None):
...
def genAggregateInteractionDictionary(EXP_REP_DICT_PATH, state_decompositions, gpcrDbDict, CHAIN_ID=None, POLAR_NETWORK_PATH=None):
	...
	for interaction_type in INTERACTION_LIST:
		...
		rLabel, rawBinMat = parseInteractionDictionary(state_decompositions, interaction_type, EXP_REP_INTERACTION_DICT_PATH, gpcrDbDict, CHAIN_ID, POLAR_NETWORK_PATH)
		rowLabels += rLabel
...
def interDictToFreqDriver(EXP_REP_DICT_PATH, STATE_DECOMP_FILE, OUTPUT_FILE_PATH, INTERACTION_LIST, gpcrDbDict, CHAIN_ID=None, POLAR_NETWORK_PATH = None):
	"""
	...
	aggRowLabels, aggRawBinaryMatrix = genAggregateInteractionDictionary(EXP_REP_DICT_PATH, state_decompositions, gpcrDbDict, CHAIN_ID, POLAR_NETWORK_PATH)
	...
	parseStateFrequencies(fw, unionRowLabels, unionRawBinaryMatrix, state_decompositions, POLAR_NETWORK_PATH)
...
if __name__ == "__main__":
	...
	### Driver Method
	runTime = interDictToFreqDriver(EXP_REP_DICT_PATH, STATE_DECOMP_FILE, OUTPUT_FILE_PATH, INTERACTION_LIST, gpcrDbDict, CHAIN_ID, POLAR_NETWORK_PATH)
