...
# Example
AGGREGATE_FREQ_TABLE="/scratch/PI/rondror/akma327/DynamicNetworks/results/simulation-analysis/gpcrdb-freq-output/aggregate-freqsum/051216-3.5A-70D-full-aggregate.txt"
...
PDB_CODE="4N6H"
OUTPUT_FILE="/scratch/PI/rondror/akma327/DynamicNetworks/results/interaction-footprint-analysis/polar-network-freq-tables/atom-resolution/4N6H/dor-inactive-footprint.txt"
...
def inGPCRDB_Set(GPCRDB_SET, gpcrdb1, gpcrdb2, interaction_type):
	key1 = (gpcrdb1, gpcrdb2, interaction_type)
	key2 = (gpcrdb2, gpcrdb1, interaction_type)
	if(key1 in GPCRDB_SET or key2 in GPCRDB_SET):return True 
	else: return False 
...
def subsetAggregateFreqTable(AGGREGATE_FREQ_TABLE, GPCRDB_SET, OUTPUT_FILE):
	...
	for line in f: 
		...
		gpcrdb1, gpcrdb2, interaction_type = interaction_label[0].strip(), interaction_label[1].strip(), interaction_label[4].strip()
		if(inGPCRDB_Set(GPCRDB_SET, gpcrdb1, gpcrdb2, interaction_type) or inGPCRDB_Set(GPCRDB_SET, gpcrdb2, gpcrdb1, interaction_type)):
...
def sortByInteractionFlag(subset_list):
	...
	for si in subset_list:
		linfo = si.split("\t")
		interaction_info = linfo[0].split(":")
		freq_info = linfo[1:]
		split_subset_list.append(tuple(interaction_info + freq_info))
	split_subset_list = sorted(split_subset_list, key= lambda x: x[4])
	sorted_subset_interaction_list = [":".join(list(linfo[0:5])) + "\t" + "\t".join(list(linfo[5:])) for linfo in split_subset_list]
...
def atomFootprintTableDriver(AGGREGATE_FREQ_TABLE, NETWORK_FILTER_LIST, PDB_CODE, OUTPUT_FILE):
...
if __name__ == "__main__":
	...
	atomFootprintTableDriver(AGGREGATE_FREQ_TABLE, NETWORK_FILTER_LIST, PDB_CODE, OUTPUT_FILE)
