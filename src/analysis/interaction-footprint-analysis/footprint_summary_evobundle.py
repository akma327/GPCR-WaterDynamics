...
# Usage 
# python footprint_summary_evobundle.py <FOOTPRINT_TABLE> <EXP_REP_ID> <OUTPUT_FILE> <-intra-pdb> <EXP_REP_PDB> <-receptor> <RECEPTOR> <-no-conserve> <-upper or -lower> <-cutoff> <CUTOFF_FREQ> <-inter> <-ligand>
...
# <-ligand> Optional flag (-ligonly or -ligand) to denote whether to include only interactions that
# involve a ligand, or to include receptor interactions and ligand mediated interactions. If this 
# flag is not provided, then defaults to receptor interactions only. 
...
def isInterHelixInteraction(name1, name2):
	tm1, i1 = name1.split(".")[1].split("x")
	tm2, i2 = name2.split(".")[1].split("x")

	### Different helix
	if(tm1 != tm2 or "LIG" in name1 or "LIG" in name2): 
		return True

	### Same helix
	else:
		index1, index2 = int(i1), int(i2)
		if(index2 not in range(index1 -3, index1 +4)):
			return True
...
def addInteractionToJSONDict(json_dict, interaction_key, freq_val, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, REGION, RECEPTOR):
	...
	### Ligand Inclusion 
	if(LIGAND_INCLUSION == "receptor_only"):
		if("LIG" in name1 or "LIG" in name2): return
	elif(LIGAND_INCLUSION == "ligand_only"):
		if("LIG" not in name1 and "LIG" not in name2): return 
...
def populateInteractionInfo(json_dict, FOOTPRINT_TABLE, EXP_REP_ID, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, REGION, RECEPTOR):
	...
	for index, interaction_key in enumerate(row_labels):
		...
		addInteractionToJSONDict(json_dict, interaction_key, freq_val, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, REGION, RECEPTOR)
...
def createJSONDict(FOOTPRINT_TABLE, EXP_REP_ID, CONSERVE, REGION, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, EXP_REP_PDB, RECEPTOR):
	json_dict = {"edges": []}
	populateInteractionInfo(json_dict, FOOTPRINT_TABLE, EXP_REP_ID, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, REGION, RECEPTOR)
...
def bundlePlotJsonDriver(FOOTPRINT_TABLE, EXP_REP_ID, OUTPUT_FILE, CONSERVE, REGION, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, EXP_REP_PDB=None, RECEPTOR = None):
	print("Processing: " + OUTPUT_FILE)
	OUT_DIR = "/".join(OUTPUT_FILE.split("/")[0:-1])
	if not os.path.exists(OUT_DIR):
		os.makedirs(OUT_DIR)

	json_dict = createJSONDict(FOOTPRINT_TABLE, EXP_REP_ID, CONSERVE, REGION, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, EXP_REP_PDB, RECEPTOR)
...
if __name__ == "__main__":
	...
	if("-inter" in sys.argv):
		INTER_ONLY = True
	LIGAND_INCLUSION = "receptor_only"
	if("-ligonly" in sys.argv):
		LIGAND_INCLUSION = "ligand_only"
	elif("-ligand" in sys.argv):
		LIGAND_INCLUSION = "receptor_and_ligand"
	bundlePlotJsonDriver(FOOTPRINT_TABLE, EXP_REP_ID, OUTPUT_FILE, CONSERVE, REGION, CUTOFF_FREQ, INTER_ONLY, LIGAND_INCLUSION, EXP_REP_PDB, RECEPTOR)
